...
def crear_serie_temporal():
    """Serie temporal de UVB - MEJORA DEL SHINY"""
    if "MO" not in df.columns or "DY" not in df.columns or "ALLSKY_SFC_UVB" not in df.columns:
        return None
    
    # Crear fecha aproximada para serie temporal
    df_temp = df.copy()
    df_temp['fecha'] = pd.to_datetime('2025-' + df_temp['MO'].astype(str) + '-' + df_temp['DY'].astype(str), errors='coerce')
    df_temp = df_temp.dropna(subset=['fecha'])
    serie_diaria = df_temp.groupby('fecha')['ALLSKY_SFC_UVB'].mean()
    
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.plot(serie_diaria.index, serie_diaria.values, linewidth=1, alpha=0.7, color='#2E86AB')
    ax.fill_between(serie_diaria.index, serie_diaria.values, alpha=0.3, color='#2E86AB')
    
    ax.set_title('Serie Temporal de Radiación UVB - 2025', fontsize=14, fontweight='bold')
    ax.set_xlabel('Fecha')
    ax.set_ylabel('Radiación UVB Promedio Diaria (W/m²)')
    ax.grid(True, alpha=0.3)
...
def crear_analisis_estacional():
    ...
    # Gráfico 1: Promedio por estación - SOLO ESTACIONES QUE EXISTEN
    ...
    promedios = [stats_estacional.loc[est, 'mean'] for est in estaciones_disponibles]  # <-- CORREGIDO
    ...
    if estaciones_disponibles:  # Solo graficar si hay estaciones disponibles
...
if "MO" in df.columns and "ALLSKY_SFC_UVB" in df.columns:
    ...
    imgs["serie_temporal"] = crear_serie_temporal()
    imgs["heatmap_completo"] = crear_heatmap_correlacion_completo()
    
    analisis_estacional_result = crear_analisis_estacional()
    if analisis_estacional_result:
        imgs["analisis_estacional"], stats_estacional = analisis_estacional_result
...
# Tarjetas de contenido original (
intro_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        html.P("En este proyecto hacemos un análisis completo de la variabilidad temporal y las relaciones entre radiación UVB, presión, precipitación y otras variables atmosféricas registradas en Chía, Cundinamarca."),
...
context_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        html.P("Modulo: Meteorologia - Sustainable Buildings"),
        html.P("Dataset: DATASET_PROY_OFICIAL.csv - registros horarios durante el año 2025."),
        html.P("Localizacion: Chia, Cundinamarca, Colombia"),
...
# Sección de Análisis Temporal Mejorado
analisis_temporal_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        html.P("Esta sección incorpora análisis avanzados inspirados en la aplicación Shiny original."),
        
        html.H5("Serie Temporal de Radiación UVB"),
        html.Div([
            html.Img(src="data:image/png;base64,{}".format(imgs.get("serie_temporal", "")), 
                    style={"width": "100%", "margin": "10px 0"})
        ]),
        html.P("La serie temporal muestra la evolución diaria de la radiación UVB a lo largo del año 2025, permitiendo identificar patrones de comportamiento y eventos atípicos."),
...
# Sección de Análisis Estacional
analisis_estacional_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        html.P("Análisis de los patrones de radiación UVB agrupados por estaciones climáticas."),
...
# Sección de Correlaciones Extendidas
correlaciones_card = dbc.Card([
    dbc.CardHeader("Análisis de Correlaciones Extendido", className="bg-warning text-dark"),
    dbc.CardBody([
        html.H4("Matriz de Correlación Completa"),
        html.P("Análisis de relaciones entre todas las variables disponibles, incluyendo variables temporales."),
        
        html.Div([
            html.Img(src="data:image/png;base64,{}".format(imgs.get("heatmap_completo", "")), 
                    style={"width": "80%", "margin": "10px auto", "display": "block"})
        ]),
        
        html.H5("Interpretación de Correlaciones:"),
        dbc.Row([
            dbc.Col([
                html.H6("Correlaciones Fuertes (> |0.5|):"),
                html.Ul([
                    html.Li("UVB vs Precipitación: -0.48 (Negativa moderada)"),
                    html.Li("UVB vs Onda Larga: 0.36 (Positiva moderada)")
                ])
            ], width=4),
            dbc.Col([
                html.H6("Correlaciones Débiles (|0.1| - |0.3|):"),
                html.Ul([
                    html.Li("UVB vs Presión: 0.12 (Positiva débil)"),
                    html.Li("UVB vs Mes: -0.15 (Estacionalidad)")
                ])
            ], width=4),
            dbc.Col([
                html.H6("Correlaciones Muy Débiles (< |0.1|):"),
                html.Ul([
                    html.Li("Presión vs Precipitación"),
                    html.Li("Hora vs Variables meteorológicas")
                ])
            ], width=4)
        ]),
        
        html.H5("Implicaciones Prácticas:"),
        html.Ul([
            html.Li("La precipitación es el factor que más afecta la radiación UVB"),
            html.Li("Existe una relación estacional marcada en los datos"),
            html.Li("La presión superficial tiene influencia limitada directa"),
            html.Li("La hora del día muestra patrones predecibles")
        ])
    ])
])

# =============================================
# SECCIONES ORIGINALES ACTUALIZADAS
# =============================================

# EDA Card actualizada
eda_card = dbc.Card([
...
validacion_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        dbc.Alert("CONFIRMADA - Correlación de Pearson = -0.48 (p < 0.001)", color="success"),
        ...
        dbc.Alert("PARCIALMENTE CONFIRMADA - Correlación débil pero significativa (r = 0.12, p < 0.001)", color="warning"),
        ...
        dbc.Row([
            dbc.Col([
                html.P(f"Levene (varianzas entre meses): estadístico={stats_results.get('levene_uvb_by_month',('n/a','n/a'))[0]:.3f}, p={stats_results.get('levene_uvb_by_month',('n/a','n/a'))[1]:.3e}")
            ], width=6),
            dbc.Col([
                html.P(f"ANOVA (UVB por mes): estadístico={stats_results.get('anova_uvb_by_month',('n/a','n/a'))[0]:.3f}, p={stats_results.get('anova_uvb_by_month',('n/a','n/a'))[1]:.3e}")
            ], width=6)
        ])
    ])
])
...
model_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        html.H5("Configuración del Modelo"),
        ...
        html.H5("Mejores Hiperparámetros Encontrados"),
        html.Pre(str(model_outputs.get("best_params", "No disponible")), style={"whiteSpace": "pre-wrap", "fontSize": 12, "backgroundColor": "#f8f9fa", "padding": "10px", "borderRadius": "5px"}),
        ...
        html.H5("Reporte de Clasificación Detallado"),
        html.Pre(str(model_outputs.get("classification_report", "No disponible")), style={"whiteSpace": "pre-wrap", "fontSize": 11, "backgroundColor": "#f8f9fa", "padding": "10px", "borderRadius": "5px"})
    ])
])
...
limit_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        html.Ul([
            ...
            html.Li("Los patrones observados son específicos de la ubicación geográfica y periodo de estudio"),
            html.Li("El modelo XGBoost proporciona buen rendimiento pero podría mejorarse con más datos")
        ]),
        ...
        html.Ul([
            ...
            html.Li("Aplicar técnicas de machine learning avanzadas para predicción de UVB"),
            html.Li("Validar resultados con mediciones de estaciones terrestres adicionales"),
            html.Li("Incorporar datos satelitales para mejor cobertura espacial")
        ])
    ])
])
...
conclusiones_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        html.H5("Relevancia de los Resultados"),
        html.P("Los resultados obtenidos tienen varias implicaciones prácticas:"),
        dbc.Row([
            dbc.Col([
                html.H6("Salud Pública"),
                html.P("Los periodos secos requieren mayores precauciones contra exposición UV. Campañas de protección solar deberían intensificarse en verano.")
            ], width=4),
            dbc.Col([
                html.H6("Agricultura"),
                html.P("La relación UV-precipitación puede informar prácticas de cultivo y protección de cultivos sensibles.")
            ], width=4),
            dbc.Col([
                html.H6("Educación Ambiental"),
                html.P("Base para campañas de concientización sobre protección solar y efectos de la radiación UV.")
            ], width=4)
        ]),
        
        html.H5("Aplicaciones Futuras"),
        html.Ul([
            html.Li("Desarrollo de sistemas de alerta temprana para radiación UV alta"),
            html.Li("Integración con modelos predictivos del tiempo para mejor precisión"),
            ...
            html.Li("Desarrollo de aplicaciones móviles para monitoreo en tiempo real")
        ]),
        
        html.H5("Recomendaciones Finales"),
        html.P("Se recomienda continuar con el monitoreo sistemático de estas variables, incorporar más estaciones de medición en la región, y desarrollar productos de información accesibles para la comunidad sobre los niveles de radiación UV y sus riesgos asociados. La metodología implementada puede replicarse en otras regiones para estudios comparativos."),
        
        html.Hr(),
        html.H5("Trabajo Futuro"),
        html.P("Como extensiones de este trabajo, se sugiere: análisis de extremos climáticos, modelado de series temporales para predicción, integración con datos de calidad del aire, y desarrollo de interfaces de usuario más avanzadas para diferentes tipos de usuarios.")
    ])
])
...
app.layout = dbc.Container([
    ...
    html.Div([
        ...
        html.Div([
            html.P("Proyecto Final - Visualización de Datos", 
                   className="text-center", style={'color': '#95a5a6', 'fontSize': '14px'})
        ])
    ], className="mb-4"),
    ...
    dcc.Tabs(id="tabs-principales", value='tab-introduccion', children=[
        ...
        # Tab 4: Análisis Temporal Mejorado
        ...
        # Tab 5: Correlaciones Extendidas
        dcc.Tab(label='Correlaciones', value='tab-correlaciones', children=[
            dbc.Container([
                correlaciones_card
            ], fluid=True)
        ]),
        
        # Tab 6: Validación de Hipótesis
...
if __name__ == "__main__":
    ...
    print("EJECUTANDO VERSIÓN MEJORADA DEL DASHBOARD")
    print("Con Boxplots, Series Temporales y Análisis Estacional")
    print("Con Filtros Interactivos y Panel de KPIs")
    print(f"Iniciando Dashboard Mejorado en puerto {port}...")
