...
server = app.server
app.title = "Análisis de Radiación UVB en Chía"
...
correlaciones_card = dbc.Card([
    dbc.CardHeader("Análisis de Correlaciones", className="bg-warning text-dark"),
    dbc.CardBody([
...
model_card = dbc.Card([
    ...
    dbc.CardBody([
        ...
        # FÓRMULA MATEMÁTICA XGBOOST
        dbc.Card([
            dbc.CardHeader("Fórmula Matemática del Modelo XGBoost", className="bg-info text-white"),
            dbc.CardBody([
                html.H5("Función Objetivo:"),
                html.P("L(ϕ) = Σᵢ[l(yᵢ, ŷᵢ)] + Σₖ[Ω(fₖ)]", style={"fontFamily": "monospace", "fontSize": "18px", "textAlign": "center"}),
                
                dbc.Row([
                    dbc.Col([
                        html.H6("Donde:"),
                        html.Ul([
                            html.Li("l(yᵢ, ŷᵢ): Función de pérdida (log-loss binaria)"),
                            html.Li("Ω(fₖ) = γT + ½λ‖w‖²: Término de regularización"),
                            html.Li("T: Número de hojas en el árbol"),
                            html.Li("w: Vector de scores en las hojas"),
                            html.Li("γ, λ: Parámetros de regularización")
                        ])
                    ], width=6),
                    dbc.Col([
                        html.H6("Predicción:"),
                        html.P("ŷᵢ = σ(Σₖ fₖ(xᵢ))", style={"fontFamily": "monospace", "fontSize": "16px"}),
                        html.P("σ(z) = 1 / (1 + e⁻ᶻ) - Función sigmoide", style={"fontFamily": "monospace", "fontSize": "14px"}),
                        html.Ul([
                            html.Li("HighUVB ∈ {0, 1} - Variable objetivo"),
                            html.Li("Parámetros optimizados: n_estimators, max_depth, learning_rate, subsample")
                        ])
                    ], width=6)
                ])
            ])
        ...
        html.H5("Parámetros del Modelo Optimizados", className="mt-4"),
        dbc.Row([
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H6("Grid Search CV", className="card-title"),
                        html.P("Validación Cruzada Estratificada (5 folds)"),
                        html.P("Métrica: F1-Macro"),
                        html.P(f"Mejor Score CV: {model_outputs.get('best_cv_score', 0):.3f}")
                    ])
                ])
            ], width=6),
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        html.H6("Hiperparámetros", className="card-title"),
                        html.P(f"n_estimators: {model_outputs.get('best_params', {}).get('model__n_estimators', 'N/A')}"),
                        html.P(f"max_depth: {model_outputs.get('best_params', {}).get('model__max_depth', 'N/A')}"),
                        html.P(f"learning_rate: {model_outputs.get('best_params', {}).get('model__learning_rate', 'N/A')}"),
                        html.P(f"subsample: {model_outputs.get('best_params', {}).get('model__subsample', 'N/A')}")
                    ])
                ])
            ], width=6)
        ])
